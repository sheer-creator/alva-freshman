...
function scoreBreakdown(hit){return `语义 ${hit.semantic.toFixed(4)} + 实体 ${hit.entityBonus.toFixed(4)} + 新鲜度 ${hit.freshnessBonus.toFixed(4)} ≈ ${hit.score.toFixed(4)}`}
function scoreReference(hit){const t=data.theses[hit.sourceThesisId];return t?`${t.catalog.author_name} · ${t.catalog.tickers.join(' / ')}`:'当前观点'}
...
function card(item){const t=item.thesis;if(!t||!t.catalog)return null;const s=t.catalog,uid=user;const el=document.createElement('article');el.className='card';el.dataset.key=item.itemKey;el.dataset.user=uid;el.thesis=t;
 ...
 const main=el.querySelector('.card-main');main.onclick=()=>openThesis(t,item.recommendationOrigin);main.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openThesis(t,item.recommendationOrigin)}};
 el.querySelector('[data-action="open"]').onclick=()=>openThesis(t,item.recommendationOrigin);
