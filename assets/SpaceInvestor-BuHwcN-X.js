import{j as t,P as n}from"./index-DpzxiPjJ.js";import{A as a}from"./AppShell-CuheVZS3.js";import{i}from"./inlinePlaybookHeader-Cld2A8Gm.js";import"./referral-mock-DJxnAaFy.js";const s=`<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  ...
  <style>
    ...
    *::-webkit-scrollbar { display: none; }
    ...
    .stack { display: flex; flex-direction: column; gap: var(--spacing-xxl); }    .grid-gap { margin-top: 0; }
    ...
    .tooltip { position: absolute; display: flex; flex-direction: column; gap: var(--spacing-xxxs); width: max-content; max-width: 400px; padding: var(--spacing-m); background-color: var(--b0-container); border-radius: var(--radius-pop-tips); box-shadow: var(--shadow-s); }
    ...
    .ct-info:hover .tooltip, .ct-info:focus-visible .tooltip, .ct-info.tip-open .tooltip { opacity: 1; visibility: visible; transform: translateY(0); }
    @media (hover: none) { .ct-info:hover .tooltip { opacity: 0; visibility: hidden; } }  /* touch: tap opens it, hover must not */
    ...
    .modal-sheet { display: flex; flex-direction: column; gap: var(--spacing-m); width: 100%; max-width: 680px; max-height: 100%; padding: var(--spacing-xxl); background: var(--b0-container); border: 0.5px solid var(--line-l2); border-radius: var(--radius-pop-dialog, 8px); }
    ...
    .pb-lock-ic { width: 16px; height: 16px; flex-shrink: 0; }
    body.unlocked .pb-lock-ic { display: none; }
    .pb-unlock-ic { display: none; margin-left: 0; flex-shrink: 0; }
    /* markdown link underline (design-components.md .markdown-container a): dotted n5, 8% / 35%, hover m1 */
    .tooltip-link { color: inherit; font-weight: 500; text-decoration: underline dotted var(--text-n5, rgba(0,0,0,0.5)); text-decoration-thickness: 8%; text-underline-offset: 35%; text-decoration-skip-ink: none; cursor: pointer; transition: color 0.15s ease; }
    .tooltip-link:hover { color: var(--main-m1, #49A3A6); text-decoration-color: var(--main-m1, #49A3A6); }
    .pb-unlock-ic img { width: 16px; height: 16px; display: block; }
    ...
    .pb-btn-subscribe .n { font-weight: 400; color: var(--text-nr5, rgba(255,255,255,0.5)); }
    ...
    .paywall { position: absolute; inset: 0; z-index: 50; display: flex; flex-direction: column; overflow: hidden; background: var(--b0-container, #fff); }
    ...
    .paywall-feed { flex: 1; min-height: 0; display: flex; align-items: center; justify-content: center; padding: 28px; overflow: auto; }
    /* Middle content cap — Figma "Max" 13381:295002 */
    .pw-max { display: flex; align-items: center; gap: 28px; width: 100%; max-width: 1080px; }
    /* Purchase panel — Figma 13381:294657 */
    .purchase { display: flex; flex-direction: column; gap: 32px; flex: 1; min-width: 0; }
    .pu-lock { width: 64px; height: 64px; display: flex; align-items: center; justify-content: center; background: #fff; border: 0.5px solid var(--line-l3, rgba(0,0,0,0.3)); border-radius: var(--radius-ct-xl, 12px); }
    .pu-lock img { width: 32px; height: 32px; display: block; }
    .pu-benefits { display: flex; flex-direction: column; gap: 16px; }
    .pu-row { display: flex; align-items: flex-start; gap: 12px; font-size: 20px; line-height: 30px; letter-spacing: 0.2px; color: var(--text-n9, rgba(0,0,0,0.9)); }
    .pu-row .ic { width: 26px; height: 26px; margin-top: 2px; flex-shrink: 0; display: block; background: var(--main-m1, #49A3A6); -webkit-mask: url(https://alva-ai-static.b-cdn.net/icons/check-f3.svg) center / contain no-repeat; mask: url(https://alva-ai-static.b-cdn.net/icons/check-f3.svg) center / contain no-repeat; }
    .pu-cta { display: flex; flex-direction: column; gap: 16px; align-items: flex-start; }
    ...
    .pw-msg:not(:last-child) { margin-bottom: -108px; }
    .pw-msg.h200 { height: 220px; }
    ...
    .pw-bubble { position: relative; flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 8px; height: 220px; overflow: hidden; padding: 12px; background: var(--b0-container, #fff); border: 0.5px solid var(--line-l2, rgba(0,0,0,0.2)); border-radius: var(--radius-ct-l, 8px); }
    .pw-bubble::after { content: ""; position: absolute; left: 0; right: 0; bottom: 0; height: 40px; background: linear-gradient(180deg, rgba(255,255,255,0) 0%, #fff 100%); pointer-events: none; }
    /* top card carries no shadow; the two beneath cast 0 -6 20 5% upward */
    .pw-msg.op50 .pw-bubble, .pw-msg:last-child .pw-bubble { box-shadow: 0 -6px 20px rgba(0,0,0,0.05); }

    /* ── locked tail (13947:114079 + 13947:114297) — the stack keeps going, but behind the paywall ── */
    .pw-collapsed { display: flex; flex-direction: column; align-items: center; padding-left: 32px; margin-top: -12px; }
    .pw-collapsed i { display: block; height: 8px; background: var(--b-common-white, #fff); border: 0.5px solid var(--line-l2, rgba(0,0,0,0.2)); border-top: 0; border-radius: 0 0 4px 4px; transform-origin: top center; }
    .pw-collapsed .s2 { width: 400px; max-width: 100%; z-index: 2; }
    .pw-collapsed .s3 { width: 340px; max-width: calc(100% - 60px); z-index: 1; }
    .pw-newer { display: flex; align-items: center; gap: 12px; height: 20px; }
    .pw-newer .ln { flex: 1; min-width: 0; height: 0; border-top: 0.5px solid var(--line-l12, rgba(0,0,0,0.12)); transform-origin: center; }
    .pw-newer .pill { flex-shrink: 0; display: inline-flex; align-items: center; gap: 4px; font-size: 12px; line-height: 20px; letter-spacing: 0.12px; color: var(--text-n5, rgba(0,0,0,0.5)); white-space: nowrap; }
    .pw-newer .pill .ic { width: 14px; height: 14px; flex-shrink: 0; background: currentColor; -webkit-mask: url(https://alva-ai-static.b-cdn.net/icons/locked-l.svg) center / contain no-repeat; mask: url(https://alva-ai-static.b-cdn.net/icons/locked-l.svg) center / contain no-repeat; }
    /* entrance: a 4th, quieter beat after the three cards — hint, not payload */
    .pw-collapsed, .pw-newer { animation: pwTailIn 0.45s ease-out 0.9s both; }
    @keyframes pwTailIn { from { opacity: 0; transform: translateY(2px); } to { opacity: 1; transform: none; } }
    /* unlock: the seal gives way before the wipe reaches this corner */
    .paywall.revealing .pw-collapsed i { opacity: 0; transform: translateY(6px) scaleY(0.4); transition: opacity 0.3s ease, transform 0.3s ease; }
    .paywall.revealing .pw-collapsed .s3 { transition-delay: 0.06s; }
    .paywall.revealing .pw-newer .pill { opacity: 0; transition: opacity 0.22s ease; }
    .paywall.revealing .pw-newer .ln { opacity: 0; transform: scaleX(1.35); transition: opacity 0.3s ease, transform 0.3s ease; }
    ...
    .pwb-so { position: relative; width: 24px; height: 24px; border-radius: 50%; border: 0.625px solid #fff; overflow: hidden; flex-shrink: 0; }
    ...
    .pwb-btn { position: relative; display: flex; align-items: center; justify-content: center; width: 240px; height: 48px; padding: 11px 20px; border: 0; border-radius: var(--radius-btn-m, 6px); background: var(--main-m1, #49A3A6); color: #fff; font-family: inherit; font-size: 16px; line-height: 26px; letter-spacing: 0.16px; font-weight: 500; white-space: nowrap; cursor: pointer; }
    ...
    .unlock-toast { position: fixed; left: 50%; top: 28px; z-index: 80; display: none; align-items: flex-start; gap: 8px; max-width: 90vw; padding: 12px 16px; background: #fff; border: 0.5px solid var(--line-l2, rgba(0,0,0,0.2)); border-radius: 4px; box-shadow: 0 6px 20px rgba(0,0,0,0.04); }
    .unlock-toast img { width: 20px; height: 20px; display: block; flex-shrink: 0; margin-top: 1px; }
    .ut-text { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
    .ut-title { font-size: 14px; line-height: 22px; letter-spacing: 0.14px; color: var(--text-n9, rgba(0,0,0,0.9)); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .ut-sub { font-size: 12px; line-height: 20px; letter-spacing: 0.12px; color: var(--text-n5, rgba(0,0,0,0.5)); }
    ...
    /* ═══════ Mobile — official mobile frame (Figma 13433:81493) ═══════ */
    .mw-topbar, .mw-dock { display: none; }
    .sp-scrim { position: fixed; inset: 0; z-index: 65; display: none; background: rgba(20,20,28,0.38); opacity: 0; transition: opacity 0.3s ease; }
    .sp-grab { display: none; }

    /* Two columns survive while Left (flex:1) can shrink: its widest unbreakable
       row is benefit line 3 at 20px (~350px incl. icons), so stack below
       28+350+28+480+28 ≈ 920. */
    @media (max-width: 940px) {
      .paywall-feed { align-items: flex-start; padding: 28px 28px 40px; }
      .pw-max { flex-direction: column; align-items: stretch; gap: 32px; max-width: 560px; margin: 0 auto; }
      .alerts-stack { width: 100%; }
    }

    @media (max-width: 640px) {
      /* ── Mweb topbar (13433:81505) replaces the desktop info header ── */
      .pb-info { display: none; }
      .mw-topbar { display: flex; align-items: center; flex-shrink: 0; height: 56px; padding-right: 8px; background: var(--b0-container, #fff); }
      .mw-back { display: flex; align-items: center; height: 100%; padding: 0 12px 0 16px; border: 0; background: transparent; cursor: pointer; }
      .mw-back img { width: 20px; height: 20px; display: block; }
      .mw-title { flex: 1; min-width: 0; font-size: 16px; line-height: 26px; letter-spacing: 0.16px; color: var(--text-n9, rgba(0,0,0,0.9)); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      .mw-actions { display: flex; align-items: center; height: 100%; }
      .mw-btn { display: flex; align-items: center; justify-content: center; gap: 4px; min-width: 40px; height: 56px; padding: 0 8px; border: 0; background: transparent; font-family: inherit; font-size: 12px; line-height: 20px; letter-spacing: 0.12px; color: var(--text-n9, rgba(0,0,0,0.9)); cursor: pointer; }
      .mw-btn img { width: 16px; height: 16px; display: block; }

      /* ── action dock (13433:81511): white / 0.5 l2 / r8 / shadow 0 4 15 12% ── */
      .mw-dock { position: fixed; left: 50%; transform: translateX(-50%); bottom: calc(16px + env(safe-area-inset-bottom, 0px)); z-index: 56; display: flex; padding: 8px 0 6px; background: #fff; border: 0.5px solid var(--line-l2, rgba(0,0,0,0.2)); border-radius: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.12); }
      .mw-dock-item { display: flex; flex-direction: column; align-items: center; justify-content: flex-end; gap: 0; min-width: 80px; height: 40px; padding: 0 12px 2px; border: 0; background: transparent; font-family: inherit; cursor: pointer; }
      .mw-dock-item img { width: 20px; height: 20px; display: block; }
      .mw-dock-item .lb { font-size: 10px; line-height: 16px; letter-spacing: 0.1px; color: var(--text-n9, rgba(0,0,0,0.9)); white-space: nowrap; }
      .mw-dock-item .lb .n { color: var(--text-n5, rgba(0,0,0,0.5)); }

      /* ── widget internals wrap; tables scroll ── */
      .chart-flex { flex-wrap: wrap; }
      .chart-rows { width: 100%; height: auto; flex-shrink: 1; }
      .trade-table { grid-template-columns: 72px 1fr 52px 60px 76px minmax(84px, 1fr); min-width: 520px; }
      .modal-body { overflow-x: auto; -webkit-overflow-scrolling: touch; }
      .modal-sheet { padding: var(--spacing-l); }
      .tab { overflow-x: auto; -webkit-overflow-scrolling: touch; }
      .pb-scroll { -webkit-overflow-scrolling: touch; }
      .ident-avatar { width: 40px; height: 40px; }
      .ident { gap: var(--spacing-s); }

      /* ── paywall — Max 13433:84007: p 40/20, col gap 40 ── */
      .paywall-feed { padding: 16px 16px 110px; -webkit-overflow-scrolling: touch; }
      .pw-max { gap: 28px; max-width: none; margin: 0; }
      .purchase { gap: 20px; }
      .pu-benefits { gap: 8px; }
      .pu-lock { width: 48px; height: 48px; }
      .pu-lock img { width: 26px; height: 26px; }
      .pu-benefits { gap: 8px; }
      .pu-row { gap: 8px; font-size: 16px; line-height: 26px; letter-spacing: 0.16px; color: var(--text-n9, rgba(0,0,0,0.9)); }
      .pu-row .ic { width: 20px; height: 20px; margin-top: 3px; }
      .pwb-so { width: 22px; height: 22px; }
      .pu-cta { gap: 12px; }
      .pu-price .v { font-size: 28px; line-height: 38px; letter-spacing: 0.28px; }
      .pu-price .u { font-size: 16px; line-height: 26px; letter-spacing: 0.16px; }
      .pu-cta { width: 100%; }
      .pwb-btn { width: 100%; }

      /* ── alerts (13433:84029, revised): a single full-width newest card, 353×300
         at the frame; the older two are gone from the mobile spec ── */
      .paywall-kicker { text-align: left; }
      .alerts-feed { display: block; }
      .pw-ava { display: none; }
      .pw-msg:not(:last-child) { display: none; }
      .pw-msg:last-child { width: 100%; height: auto; animation-delay: 0.15s; }
      .pw-bubble { height: auto; box-shadow: 0 -6px 20px rgba(0,0,0,0.05); }
      .pw-bubble::after { content: none; }   /* card is sized to its content — nothing to crop */
      .pw-collapsed { padding-left: 0; }
      .pw-collapsed .s2 { width: 88%; }
      .pw-collapsed .s3 { width: 74%; max-width: none; }
      .pw-newer { gap: 8px; }

      /* ── Subscribed popover → sheet (kept from the v7 pass) ── */
      .sp-scrim { display: block; pointer-events: none; }
      .sp-scrim.open { opacity: 1; pointer-events: auto; }
      .sub-popover { display: flex; pointer-events: none; right: 0; left: 0; bottom: 0; top: auto !important; width: auto; max-width: none;
        height: auto !important; max-height: 88vh; padding: 0 16px calc(env(safe-area-inset-bottom, 0px));
        border-radius: var(--radius-pop-action-sheets, 8px) var(--radius-pop-action-sheets, 8px) 0 0; border-bottom: 0;
        transform: translateY(106%); transition: transform 0.36s cubic-bezier(0.22, 0.61, 0.36, 1); }
      .sub-popover.open { transform: none; pointer-events: auto; animation: none; }
      .sp-grab { display: block; flex-shrink: 0; width: 36px; height: 4px; margin: 8px auto 4px; border-radius: 2px; background: var(--line-l12, rgba(0,0,0,0.12)); }
      .sp-title { padding-top: 4px; }
      .sp-close { width: 44px; height: 44px; margin: -14px; display: flex; align-items: center; justify-content: center; }
      .sp-close img { width: 16px; height: 16px; }
      .sp-cards { padding-bottom: 200px; -webkit-overflow-scrolling: touch; }
      .sp-auto { flex-wrap: wrap; gap: 8px 12px; min-height: 44px; }
      .sp-auto .name { flex-basis: calc(100% - 44px); }
      .sp-chan { margin-left: 44px; min-height: 32px; }
      .sp-footer { left: 16px; right: 16px; padding-bottom: calc(16px + env(safe-area-inset-bottom, 0px)); }
      .sp-imbtn { height: 44px; }

      /* ── toast / tooltip ── */
      .unlock-toast { left: 16px; right: 16px; transform: none; max-width: none; }
      .ut-title { white-space: normal; }
      .unlock-toast.show { animation: toastInM 0.25s ease-out both; }
      .unlock-toast.hide { animation: toastOutM 0.25s ease-in both; }
      @keyframes toastInM { from { opacity: 0; transform: translateY(-100%); } to { opacity: 1; transform: none; } }
      @keyframes toastOutM { from { opacity: 1; transform: none; } to { opacity: 0; transform: translateY(-100%); } }
      .ct-info .tooltip { left: auto; right: -4px; max-width: calc(100vw - 48px); }
    }

    /* Touch: press feedback replaces hover */
    @media (hover: none) {
      .pwb-btn:active, .pb-btn-subscribe:active, .sp-imbtn:active { opacity: 0.85; }
      .pb-chip:active, .pb-pill:active, .pb-iconbtn:active, .sp-close:active, .mw-btn:active, .mw-back:active, .mw-dock-item:active { opacity: 0.7; }
    }
  </style>
</head>
<body>
...
<header class="mw-topbar">
  <button class="mw-back" type="button" aria-label="Back"><img src="https://alva-ai-static.b-cdn.net/icons/arrow-left-l1.svg" alt="" /></button>
  <span class="mw-title">Space Investor Master Playbook</span>
  <span class="mw-actions">
    <button class="mw-btn" type="button" aria-label="About"><img src="https://alva-ai-static.b-cdn.net/icons/explain-l.svg" alt="" /></button>
    <button class="mw-btn" type="button" aria-label="Comments"><img src="https://alva-ai-static.b-cdn.net/icons/chat-l1.svg" alt="" />12</button>
    <button class="mw-btn" type="button" aria-label="Share"><img src="https://alva-ai-static.b-cdn.net/icons/share-l.svg" alt="" /></button>
  </span>
</header>
  ...
  <div class="pb-head">
    <div class="pb-row-top">
      <div class="pb-row-top-l">
        ...
        <img class="pb-lock-ic" src="https://alva-ai-static.b-cdn.net/icons/locked-f.svg" alt="Locked" />
        <span class="ct-info pb-unlock-ic" tabindex="0" aria-label="Purchased playbook">
          <img src="https://alva-ai-static.b-cdn.net/icons/unlocked-f.svg" alt="" />
          <div class="tooltip"><div class="tooltip-border"></div><div class="tooltip-text">You've unlocked this playbook. View it anytime under <a class="tooltip-link" href="#user-profile?tab=purchased" onclick="window.top.location.hash='user-profile?tab=purchased';return false;">Purchased playbooks</a> in your profile.</div></div>
        </span>
      </div>
...
<div class="pb-body">
    ...
    <div class="paywall-feed">
      ...
      <div class="purchase">
        ...
        <div class="pu-benefits">
          <div class="pu-row"><span class="ic"></span><span>Full live dashboard</span></div>
          <div class="pu-row"><span class="ic"></span><span>Real-time alerts as every signal fires</span></div>
          <div class="pu-row"><span class="ic"></span><span>Delivered to Alva and your IM</span>
      ...
      <div class="alerts-stack">
        <div class="paywall-kicker">Recently delivered alerts</div>
        ...
        <div class="pw-msg">
          ...
          <div class="pw-bubble">
            ...
            <div class="pw-list">
              ...
              <div class="pw-li">Watch next: LUNR IM-3 mission window opens the second week of June.</div>
            </div>
          </div>
        </div>
        </div>
        <div class="pw-collapsed" aria-hidden="true"><i class="s2"></i><i class="s3"></i></div>
        <div class="pw-newer">
          <span class="ln"></span>
          <span class="pill"><i class="ic"></i>Unlock to view the latest alerts</span>
          <span class="ln"></span>
        </div>
      </div>
      </div>
  ...
  <!-- mweb action dock — Figma 13433:81511 -->
  <div class="mw-dock">
    <button class="mw-dock-item" type="button"><img src="https://alva-ai-static.b-cdn.net/icons/chat-ai-l.svg" alt="" /><span class="lb">Ask Alva</span></button>
    <button class="mw-dock-item" type="button"><img src="https://alva-ai-static.b-cdn.net/icons/remix-l.svg" alt="" /><span class="lb">Remix <span class="n">1.2K</span></span></button>
    <button class="mw-dock-item" id="mw-dock-sub" type="button"><img src="https://alva-ai-static.b-cdn.net/icons/notification-l.svg" alt="" /><span class="lb">Subscribe <span class="n">512</span></span></button>
  </div>
  ...
  <div class="unlock-toast" id="unlock-toast" role="status">
    ...
    <div class="ut-text">
      <span class="ut-title">Playbook unlocked</span>
      <span class="ut-sub">View it anytime under Purchased playbooks in your profile</span>
    </div>
  </div>
</div><!-- /.pb-body -->

<!-- ═══════════ SUBSCRIBED POPOVER — Popup/Get Alerts 13381:273369 ═══════════ -->
<div class="sp-scrim" id="sp-scrim"></div>
<div class="sub-popover" id="sub-popover">
  <div class="sp-grab" aria-hidden="true"></div>
  <div class="sp-title">
  ...
  <script>
    ...
    function positionPopover() {
      if (window.matchMedia("(max-width: 640px)").matches) {   // bottom sheet — CSS owns the geometry
        pwPopover.style.top = pwPopover.style.height = "";
        return;
      }
    ...
    var pwScrim = document.getElementById("sp-scrim");
    function openPopover() { positionPopover(); pwPopover.classList.add("open"); pwScrim.classList.add("open"); }
    function closePopover() { pwPopover.classList.remove("open"); pwScrim.classList.remove("open"); }
    ...
    function runReveal() {
      // Mobile GPUs choke on a per-frame radial mask over stacked backdrop filters.
      if (window.matchMedia("(max-width: 640px), (hover: none)").matches) {
        pwPaywall.style.transition = "opacity 0.45s ease";
        pwPaywall.style.opacity = "0";
        setTimeout(function () { pwPaywall.style.display = "none"; openPopover(); }, 460);
        return;
      }
    ...
    function startUnlock() {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () { pwToast.classList.add("hide"); }, 6200);
        setTimeout(function () { pwToast.style.display = "none"; }, 6500);
      }, 1200);
    }
    pwButtons.forEach(function (b) { b.addEventListener("click", startUnlock); });
    document.getElementById("mw-dock-sub").addEventListener("click", startUnlock);
    ...
    pwScrim.addEventListener("click", closePopover);

    // Touch devices have no hover, so the info tooltips need an explicit tap toggle.
    if (window.matchMedia("(hover: none)").matches) {
      document.addEventListener("click", function (e) {
        var hit = e.target.closest ? e.target.closest(".ct-info") : null;
        document.querySelectorAll(".ct-info.tip-open").forEach(function (el) { if (el !== hit) el.classList.remove("tip-open"); });
        if (hit && !e.target.closest(".tooltip a")) { e.preventDefault(); hit.classList.toggle("tip-open"); }
      });
    }
...
`,o=i(s);function l(){return t.jsx("div",{className:"h-[calc(100dvh-56px)] lg:h-screen flex flex-col",style:{background:"var(--b0-page)"},children:t.jsx("div",{className:"flex-1 overflow-hidden",children:t.jsx("iframe",{srcDoc:o,title:n["space-investor"],className:"block h-full w-full border-0"})})})}function M({onNavigate:e}){return t.jsx(a,{activePage:"space-investor",onNavigate:e,children:t.jsx(l,{})})}export{M as default};
