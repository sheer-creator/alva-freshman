...
/* 铃铛仅 Subscribed（is-on）显示，计数两种态都显示
   铃铛跟随弹层里 Receive Automations Alerts 开关：开 = notification-l，关 = bell-off（is-muted）
   注：CDN 暂无 notification-off-l.svg（404），先内嵌 notification-l 同笔触 + 斜线的 data-URI，CDN 上线后可替换 */
.pb-alerts-bell {
    width: 14px; height: 14px; flex-shrink: 0;
    display: none;
    background-color: var(--text-n9);
    -webkit-mask: url('https://alva-ai-static.b-cdn.net/icons/notification-l.svg') center / contain no-repeat;
            mask: url('https://alva-ai-static.b-cdn.net/icons/notification-l.svg') center / contain no-repeat;
}
...
/* 弹层恒为 connected 态（未连接空态已随「默认已连接」移除） */
...
/* Unsubscribe — 全宽 outline 按钮（Subscribe 弹层底部） */
/* btn-medium + btn-secondary 规范（src/styles/button.css）：40 高 / 9×20 / radius-btn-m / 14 medium */
...
.alerts-unsubscribe-btn:hover { border-color: var(--text-n9, rgba(0,0,0,0.9)); }
.alerts-unsubscribe-btn:active { border-color: var(--line-l3, rgba(0,0,0,0.3)); background: var(--b-r02, rgba(0,0,0,0.02)); }
...
.alerts-view-channel:hover { color: var(--text-n9); }
.alerts-view-channel-arrow {
...
`,p=`/* Alva Design Tokens — single source of truth.
...
`,c='<link rel="stylesheet" href="./styles/tokens.css" />',d='<link rel="stylesheet" href="./components/playbook-header.css" />',u='<script src="./components/playbook-header.js" defer><\/script>',g='<link rel="stylesheet" href="./components/discussion-panel.css" />',b='<script src="./components/discussion-panel.js" defer><\/script>';function e(n){return n.replace(/<\/script>/gi,"<\\/script>")}function t(n){return n.replace(/\/alva-infant\//g,"/")}function v(n){return n.replace(c,`<style>${p}</style>`).replace(d,`<style>${a}</style>`).replace(u,`<script>${e(t(i))}<\/script>`).replace(g,`<style>${r}</style>`).replace(b,`<script>${e(t(o))}<\/script>`).replace("</head>",`<style>${s}</style>
</head>`).replace("</body>",`<script>${e(l)}<\/script>
</body>`)}export{v as i};
