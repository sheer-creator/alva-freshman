...
export const NOTES = {
  common: [
    '移动 chrome 全部去掉：返回箭头 → 浏览器历史；底部 tab → 一级导航；底部动作条 → 页内头部；悬浮 ⊕ → 新建入口；底部 sheet → 弹层或页内下拉。',
    'Ask Alva 不在页内做对话：卡片和详情上的 Ask Alva 只是入口，A / B 打开右侧对话面板，C 打开右下浮层卡片，当前 thesis 自动挂成上下文。',
...
export const OVERVIEW = {
  title: 'Thesis on Web',
  subtitle: '浏览 · 创建 · 管理，在 web 上的三套信息架构',
  intro: '顶部条切方案和补丁开关。三个方案用同一套 mock 数据和同一个详情页组件，只换入口、导航和布局。',
  dirs: [
    { key: 'a', name: 'A · 挂进现有骨架', tagline: 'thesis 挂进现有的壳，不加一级入口，改动最小。', rows: [
      ['导航', '现有 Sidebar，Channels 下加一组 Theses（+ 新建）'],
      ['浏览', 'Explore 加 Theses tab；关注人在 People tab'],
      ['详情', '独立页，单列'],
      ['创建', 'Theses 组的 + → 两张卡 → 弹层编辑器'],
      ['管理', 'Profile › Theses'],
      ['Alva', '右侧对话面板（现有）'],
      ['代价', '「关注的人在说什么」没有一条流'],
    ] },
    { key: 'b', name: 'B · For You 一级入口', tagline: '加一个 For You 做个性化的 thesis 流，主动发现统一收进 Explore。', rows: [
      ['导航', 'Sidebar 第一位是 For You，Theses 组同 A'],
      ['浏览', 'For You 流；Explore 查 Theses / Playbooks / People / Tickers'],
      ['详情', '同 A'],
      ['创建', 'For You 顶部 composer，或 Theses 组的 +'],
      ['管理', 'Profile › Theses'],
      ['Alva', '右侧对话面板（现有）'],
      ['代价', '多一个一级项；关注人和订阅 playbook 两套关系要讲清'],
    ] },
    { key: 'e', short: 'C', name: 'C · 内容站 · 顶栏', tagline: '去掉 Sidebar，用内容站的壳读和写；Alva 变成右下气泡加一个整页。', rows: [
      ['导航', '顶栏 For You · Explore · Markets · Portfolio · Alva Agent'],
      ['浏览', 'For You 阅读流 + 右栏；Explore 三个 tab；Markets 落地页'],
      ['详情', '文章版式，一列'],
      ['创建', '顶栏 New Thesis → 全屏编辑器'],
      ['管理', '点头像进 Profile › Theses'],
      ['Alva', '右下气泡 → 浮层卡片，可展开到 Alva Agent 整页'],
      ['代价', '没有常驻的订阅区；Channels / Chats 要进 Alva Agent 页找'],
    ] },
  ],
  patchesIntro: '都默认关，在顶部条上切。客态公开页没有开关，用深链 ?guest=1 看。',
  patches: [
    ['补丁', '方案', '做法', '参考'],
    ['新建入口', 'A · B', '四档对照：小标题 +、导航项、组内首行、双 CTA', '小红书 · Substack · Medium'],
    ['详情右栏', 'A · B', '对话框关着时，详情右栏放 Related、相关的人和 Tickers', 'Reddit · 知乎 · X'],
    ['顶部快速发表', 'C', 'For You 筛选条下常驻发帖框，发完留在当前页', 'X'],
    ['三列布局', 'C', '导航改成左侧竖排一列，中间内容，右栏顶部放搜索', 'X · 微博'],
    ['客态公开页', '全部', 'thesis、作者、ticker 三页精简壳，其余页面是登录门', 'Medium · X · Reddit · 知乎'],
  ],
  open: [
    'New thesis 入口的分量：A / B 现在是 Theses 组小标题的 +，补丁里有三档更重的做法待选。',
    '四种「订」：关注人、订阅 playbook、收藏 thesis、自选 ticker，哪几种进 Sidebar、哪几种只留 Profile（顶部条「订阅区」三档对照）。',
    '三种详情都接 Alva：thesis、playbook、company 各带一种上下文 chip，文案和形态要统一。',
    'Profile 的 tab 顺序：Theses 和 Playbooks 谁排第一（A / B 放第二，C 放第一），Starred / Purchased 怎么归组。',
    'Markets 的去处：A 保留搜索弹窗，B 可并进 Explore（顶部条可切），C 是顶栏一项。',
    'app 有、web 还没落点：Followers / Following 页、Watchlist、通知权限。',
  ],
  mapping: [
    ['app 屏', 'A · 挂进现有骨架', 'B · For You 一级入口', 'C · 内容站 · 顶栏'],
    ['For You tab', 'Explore › Theses tab', 'Sidebar › For You', '顶栏 For You'],
    ['⊕ 新建 / 入口 sheet', 'Theses 组的 + → 两张卡 → 弹层编辑器', 'Theses 组的 + 或 For You 顶部 composer', '顶栏 New Thesis → 全屏编辑器'],
    ['Create with Alva', 'Create with Alva 卡 → 右侧对话面板', 'composer 里的 Create with Alva → 右侧对话面板', '编辑器右侧抽屉'],
    ['详情 + 底栏三动作', '收藏 / 分享在头部，Ask Alva 开右侧面板', '同 A', '收藏 / 分享在作者行，Ask Alva 开浮层卡片'],
    ['Me › Thesis', 'Profile › Theses（第二个 tab）', '同 A', '点头像进 Profile › Theses（第一个 tab）'],
    ['··· Archive / Private', '详情头部 ··· + Profile 行级 ···', '同 A', '详情作者行 ··· + Profile 行级 ···'],
    ['Search tab', '公司搜索弹窗（只查 ticker）', '搜索弹窗查 ticker 和人；Explore 头部跨四类搜', '顶栏 Search → 弹窗查 ticker 和人'],
    ['Markets 入口', 'Sidebar Markets → 公司搜索（现状）', '合并态并进 Explore › Tickers', '顶栏 Markets 落地页'],
    ['Onboarding 选人', 'Explore › People tab', 'For You 首次进入的空态 + 右栏', '同 B'],
  ],
  mappingSame: '三个方案一样的：选 ticker 和可见性 → 编辑器内下拉；Polish → 编辑器内弹层；All updates → 上一版露一截 + View all 弹窗；Ticker › Thesis → 公司页 Theses tab；Settings → 沿用现有。',
  foot: 'demo 数据全部为 mock，人物与头像沿用 Figma 稿里的占位。行情是 Alva Arrays 日线快照（2026-03-02 → 2026-09-22 收盘，15 只 ticker），thesis 的「发布时价」取该版本日期当天的收盘。',
};
