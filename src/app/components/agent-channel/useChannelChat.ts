...
import type { Reply, Skill, SubpushItem } from '@/data/agent-channel/types';
...
export function useChannelChat({ onActivity }: UseChannelChatOpts = {}) {
  const { extra, tasks, taskThreads } = store.useChannelChatStore();
  ...
  return {
    extra, tasks, taskThreads,
    respond, onPrompt, onStartTask, onSubscribed, onDigestAutomation,
