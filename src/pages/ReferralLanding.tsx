/**
 * [INPUT]: referral-mock data
 * [OUTPUT]: 被邀请人 landing page — 独立全屏布局
 * [POS]: Pages 层 — 邀请链接落地页
 */
...
function Avatar({ name, size = 48 }: { name: string; size?: number }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: size * 0.42, color: '#fff', lineHeight: 1, fontFamily: "'Delight', sans-serif", fontWeight: 500 }}>{initial}</span>
    </div>
  );
}

/* ========== Alva 图标 (squircle mark only) ========== */

function AlvaMark({ size = 28 }: { size?: number }) {
  return (
...
export default function ReferralLanding({ onNavigate }: { onNavigate: (page: Page) => void }) {
  const { referrer, creditsPerReferral } = REFERRAL_DATA;
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div
          ...
          style={{
            ...
            background: 'radial-gradient(circle, rgba(73,163,166,0.08) 0%, transparent 70%)',
          }}
        />
        <div
          className="absolute"
          style={{
            ...
            background: 'radial-gradient(circle, rgba(73,163,166,0.05) 0%, transparent 70%)',
          }}
        />
      </div>
      ...
      {/* 主卡片 */}
      <div
        className="relative w-[400px] rounded-[20px] overflow-hidden"
        style={{
          background: '#fff',
          boxShadow: '0 1px 2px rgba(0,0,0,0.04), 0 8px 40px rgba(0,0,0,0.06)',
        }}
      >
        {/* 顶栏 — logo + 徽章 */}
        <div className="flex items-center justify-between px-[28px] pt-[28px]">
          <AlvaMark size={26} />
          <span
        ...
        <div className="px-[28px] pt-[36px] pb-[32px] flex flex-col items-center">
          ...
          <div className="mt-[20px] flex items-baseline gap-[6px]">
            ...
            <span className="text-[15px] leading-[20px]" style={{ color: 'rgba(0,0,0,0.3)' }}>
              credits each
            </span>
          </div>
          ...
          {/* OAuth 登录 */}
          <div className="mt-[28px] w-full flex flex-col gap-[10px]">
            <button
          ...
          <p className="mt-[16px] text-[11px] leading-[16px] text-center" style={{ color: 'rgba(0,0,0,0.2)' }}>
            Already on Alva?{' '}
            <span className="cursor-pointer" style={{ color: '#49A3A6' }} onClick={() => onNavigate('home')}>
              Sign in
            </span>
          </p>
        </div>
