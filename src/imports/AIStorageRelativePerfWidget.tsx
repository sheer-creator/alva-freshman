  ...
  const option = {
    tooltip: {
      ...
      formatter: function(params: { color: string; seriesName: string; data: [string, number] }[]) {
        ...
        params.forEach((p) => {
          ...
          html += `<div style="font-family:'Delight',-apple-system,BlinkMacSystemFont,sans-serif;font-size:12px;font-weight:400;color:rgba(0,0,0,0.9);line-height:20px">${dot}${p.seriesName}: ${p.data[1].toFixed(1)}</div>`;
        });
        return html;
      }
    },
    ...
    series: [
      ...
      },
      {
        ...
        showSymbol: false,
        smooth: 0.1,
        lineStyle: { width: 1, color: '#40A544' },
        itemStyle: { color: '#40A544' },
        emphasis: {
          itemStyle: {
            borderColor: '#ffffff',
            borderWidth: 1,
      ...
      },
      {
        ...
        showSymbol: false,
        smooth: 0.1,
        lineStyle: { width: 1, color: '#5F75C9' },
        itemStyle: { color: '#5F75C9' },
        emphasis: {
          itemStyle: {
            borderColor: '#ffffff',
            borderWidth: 1,
